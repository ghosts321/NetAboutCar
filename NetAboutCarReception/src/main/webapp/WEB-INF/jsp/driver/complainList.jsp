		...
		<div class="buss_area">
			...
			<div id="workarea" class="workarea">
				<div class="title_area">
					<div class="title">驾驶员投诉公示
					</div>
					<div style="overflow-y:hidden;overflow-x:auto;width:100%;height:530px;">
					<table width="100%" border="0" cellpadding="5" cellspacing="2" class="table_main">
								...
								<th  align="center" style="font-size:15px;">投诉驾驶证号码</th>
								<th  align="center" style="font-size:15px;">投诉车牌号</th>
								<th  align="center" style="font-size:15px;">投诉司机姓名</th>
								<th align="center" style="font-size:15px;">司机所属平台</th>
							...
							<c:forEach items="${list}" var="complain" varStatus="vs">  
								<tr>
								    <td width="80px" align="center" style="font-size:13px;">${((pagebean.pageDQ-1)*12)+(vs.index+1)}</td>
									<td width="270px" align="center" style="font-size:13px;">${complain.complain_code}</td>
									<td width="200px" align="center" style="font-size:13px;">${complain.compn_name}</td>
									<td width="320px"  align="center" style="font-size:13px;">${complain.driver_licence}</td>
									<td width="180px" align="center" style="font-size:13px;">${complain.car_number}</td>
									<td width="230px" align="center" style="font-size:13px;">${complain.driver_name}</td>
									<td width="270px" align="center" style="font-size:13px;">${complain.car_company}</td>
									<td width="250px" align="center" style="font-size:13px;">${complain.compn_reason}</td>
									<td width="300px" align="center" style="font-size:13px;">${complain.compn_time}</td>
									<td width="200px" align="center" style="font-size:13px;">
									<c:choose>
									...
									<td width="300px" align="center" style="font-size:13px;">${complain.idea_time}</td>
								</tr>
							</c:forEach>
					</table>
					</div>				
				...
				<div class="pagination">
					...
					<ul>
						<li >
							<a href="javascript:void(0);" data-page="0"  onclick="gotoBuss('${basePath}driver/openComplain.action?pageDQ=1');">首页</a>
						</li>
						 <c:choose>
						 <c:when test="${DQ==1}">
						 <li class="disabled">
						  ...
						  <c:when test="${DQ>1}">
						  <li >
							<a href="javascript:void(0);" data-page="0" onclick="gotoBuss('${basePath}driver/openComplain.action?pageDQ=${DQ-1}');">上一页</a>
						</li>
						  </c:when>
						 </c:choose>						
						<c:forEach begin="1" end="${ZYS}" varStatus="vs">						
						  <c:if test="${DQ<=3}">						  
						  <li class="active">
							<a href="javascript:void(0);" onclick="gotoBuss('${basePath}driver/openComplain.action?pageDQ=${vs.index}');" data-page="${vs.index}">${vs.index}</a>
						 </li>					 	  
						  </c:if>						
						 <c:if test="${DQ>3}">
						    ...
						    <c:if test="${DQ+vs.index-1<=pagebean.pageZYS}">
						      <li class="active">
						      <a href="javascript:void(0);" onclick="gotoBuss('${basePath}driver/openComplain.action?pageDQ=${DQ+vs.index-1}');" data-page="${vs.index}">${DQ+vs.index-1}</a>
						      </li>
						    </c:if>					 
						 </c:if>
						</c:forEach>
						 ...
						 <li class="disabled">
						  ...
						  <c:when test="${DQ<pagebean.pageZYS}">
						  <li >
							<a href="javascript:void(0);" data-page="0" onclick="gotoBuss('${basePath}driver/openComplain.action?pageDQ=${DQ+1}');">下一页</a>
						</li>
						  </c:when>
						 </c:choose>
						<li>
							<a href="javascript:void(0);" data-page="4" onclick="gotoBuss('${basePath}driver/openComplain.action?pageDQ=${pagebean.pageZYS}');">末页</a>
						</li>
					</ul>
				</div>  
				   <div align="center">
