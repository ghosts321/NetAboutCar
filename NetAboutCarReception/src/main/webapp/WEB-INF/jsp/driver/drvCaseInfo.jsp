				...
				<div class="title_area">
					...
					<form method="post" name="form1" id="form1"> 
						<table width="100%" border="0" cellpadding="5" cellspacing="2" class="table_main">
							...
							<c:forEach items="${drvCaseInfoList}" var="caseinfo" varStatus="vs">  
								<tr>
									<td width="5%" align="center">${vs.index + 1}</td>
									<td align="center">${caseinfo.caseid}</td>
									<td align="center">${caseinfo.carid}</td>
									<td align="center"><c:choose>  
					    <c:when test="${caseinfo.casesource=='01'}"> 
							     稽查执法查扣违章
					    </c:when>      
					    <c:when test="${caseinfo.casesource=='02'}"> 
							      信访投诉违章
					    </c:when>
					 </c:choose>	</td>
									<td align="center">${caseinfo.instid}</td>
									<td align="center">${caseinfo.caseuserid}</td>
									<td align="center">${caseinfo.casetime}</td>
									<td align="center">${caseinfo.updatetime}</td>
							<td width="9%" align="center"><input type="button" value="申诉" href="javascript:void(0);" onclick="gotoBuss('${basePath}driver/openappealDetail.action?caseid=${caseinfo.caseid}');"/></td>
