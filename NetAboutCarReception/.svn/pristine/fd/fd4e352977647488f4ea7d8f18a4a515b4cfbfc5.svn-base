		...
		<script type="text/javascript">
			...
			$(document).ready(function(){
				$.formValidator.initConfig({formid:"form1",debug:false,submitonce:true,
					...
					$("#form1").ajaxSubmit({
	                    type:'post',
	                    url:'${basePath}terrace/appealComit.action',
	                    success:function(data){
	                    	if(data == "success"){
	                    	alert("申诉成功");
	                    		$("#main").load("${basePath}terrace/openAppeal.action", function() {});
							}else if(data == "error"){
								alert("处理失败!");
							}
	                    }
...
background-color:#f5fafe;
		...
		<div class="buss_area">
			<div class="personalmenu">
				<jsp:include page="/WEB-INF/jsp/driver/personalMenu.jsp">
					<jsp:param name="leftmenu" value="platForm5"/>
				</jsp:include>
			</div>
			<div id="workarea" class="workarea">
				...
				<div>
					<form action="appealComit.action"  method="post" name="form1" id="form1" enctype="multipart/form-data">
						<table   border="1" class="table" >
							
							<tr>
								<td align="right"  >投诉编号：</td>
			                     <td width="80%">${sessionScope.appealList[0].complain_code}</td>	 
							</tr>
							<tr>
							  <td align="right">投诉时间：</td>
				              <td>${sessionScope.appealList[0].compn_time}
				               <input type="hidden" id="complain_code" name="complain_code" value="${appealList[0].complain_code}"/>
				              <input type="hidden" id="complain_info_id" name="complain_info_id" value="${appealList[0].complain_info_id}"/>
				              </td>
			                  ...
			                  <td  align="left">
									<c:choose>
				                    <c:when test="${appealList[0].dis_type=='0'}">不做处理</c:when>
				                    <c:when test="${appealList[0].dis_type=='1'}">口头警告</c:when>
				                    <c:when test="${appealList[0].dis_type=='2'}">培训教育</c:when>
				                    <c:when test="${appealList[0].dis_type=='3'}">退款</c:when>
				                    <c:when test="${appealList[0].dis_type=='4'}">转发行政处罚</c:when>
				                    </c:choose>
									</td>
			                  </tr> 
	...
	<button type='button' class='button3' onclick="gotoBuss('${basePath}terrace/openAppeal.action');"><span class='glyphicon glyphicon-pencil' aria-hidden='true' >返回</button>&nbsp;
