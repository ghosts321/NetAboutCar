...
var TableInit = function () {
    ...
    oTableInit.Init = function () {
        $('#tradeList').bootstrapTable({
            url: 'reviselist.action',         //请求后台的URL（*）
            method: 'post',                      //请求方式（*）
            dataType: "json",
          ...
          //  height: 420,                        //行高，如果没有设置height属性，表格自动根据记录条数觉得表格高度
            ...
            ,{
                ...
                formatter:function(value, row, index){
	                	...
	                	html += "<button type='button' class='btn btn-default' onclick='toHistory(\""+row.caseId+"\")'><span class='glyphicon glyphicon-pencil' aria-hidden='true' >修改历史</button>";
						return html;
                }
            }],
...
function toAlter(caseId){
	var diag = new top.Dialog();
	 diag.Drag=true;
	 diag.Title ="修改";
	 diag.URL = '<%=basePath%>casemanage/manageAlter.action?caseId='+caseId;
	 diag.Width = 500;
	 diag.Height = 550;
...
function toHistory(caseId){
	var diag = new top.Dialog();
	 diag.Drag=true;
	 diag.Title ="修改历史";
	 diag.URL = '<%=basePath%>casemanage/penaltyHistory.action?caseId='+caseId;
	 diag.Width =1000;
	 diag.Height = 600;
	 diag.CancelEvent = function(){ //关闭事件
	...
	<div class="panel-body">
		<div class="page-header" >
			<h1 style="font-size: 15px;">
				...
				<i class="icon-double-angle-right"></i>平台案件修改
				</small>
			</h1>
		</div>
