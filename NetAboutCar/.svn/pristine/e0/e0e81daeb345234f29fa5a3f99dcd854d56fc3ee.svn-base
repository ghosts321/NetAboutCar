...
var TableInit = function () {
    ...
    oTableInit.Init = function () {
        $('#tradeList').bootstrapTable({
            url: 'carwzwfFindAll.action',         //请求后台的URL（*）
            method: 'post',                      //请求方式（*）
            dataType: "json",
           ...
           // height: 420,                        //行高，如果没有设置height属性，表格自动根据记录条数觉得表格高度
            uniqueId: "car_illegal_id",                     //每一行的唯一标识，一般为主键列
              columns: [{
            	  field: 'index',
                  align: 'center',
                  title: '序号',
                ...
                formatter:function(value, row, index){
                	var html = "";	
                	if(row.status == 1 ){
                		html += "<button type='button' class='btn btn-default' onclick='toAccept(\""+row.car_illegal_id+"\")'><span class='glyphicon glyphicon-pencil' aria-hidden='true' >判定</button>&nbsp;";
                	}else{
                		html += "<button type='button' class='btn btn-default' disabled onclick='toAccept(\""+row.car_illegal_id+"\")'><span class='glyphicon glyphicon-pencil' aria-hidden='true' >判定</button>&nbsp;";
                	}
...
function toAccept(car_illegal_id){
	var diag = new top.Dialog();
	 diag.Drag=true;
	 diag.Title ="违规判定";
	 diag.URL = '<%=basePath%>supervise/carwzwfDetermine.action?car_illegal_id=' + car_illegal_id;
	 diag.Width = 800;
	 diag.Height = 700;
	 diag.CancelEvent = function(){ //关闭事件
		/*  if(diag.innerFrame.contentWindow.document.getElementById('zhongxin').style.display == 'none'){
	...
	<div class="panel-body">
		...
		<div class="panel panel-default">
		 ...
		 <div id="toolbar" class="btn-group" style="float:left">
			<form action="<%=basePath%>supervise/carimportExcel.action" id="importForm" method="post" enctype="multipart/form-data" style="float: left">
	            <div class="box">
			...
			<form action="<%=basePath%>supervise/carexportExcel.action" id="exportForm" method="post" enctype="multipart/form-data" style="float: left">
