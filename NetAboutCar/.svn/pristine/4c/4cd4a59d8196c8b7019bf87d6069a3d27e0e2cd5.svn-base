...
angular.module('wyc',[]).controller('WycController',function($http,$scope){
	...
	if('${event.driverCertno}'!=''){
		...
		for(var i=0;i< drvCertnos.length;i++){
		    $http.post("${path}/credit/event/driver/find.action?drvCertno="+drvCertnos[i]).success(function (response) {
		    	if(response==null){
	...
	if('${event.comCreditcode}'!=''){
		...
		for(var i=0;i< comCreditcodes.length;i++){
		    $http.post("${path}/credit/event/platform/find.action?creditcode="+comCreditcodes[i]).success(function (response) {
		    	if(response==null){
		            ...
		            <div class="panel-body">
		                ...
		                <table width="100%" border="1" cellpadding="5" cellspacing="2" class="table">
							<tr>
								...
								<td colspan="3">
									<c:choose>
										<c:when test="${null == er.result_file || er.result_file== ''}">无</c:when>
										<c:otherwise><a href="${er.result_file}">点击下载</a></c:otherwise>
									</c:choose>
								</td>
							</tr>
							<tr>
							    <th style="text-align: right;" >处理结论：</th>
							    <td colspan="3">${er.review_result }</td>
							</tr>
		                </table>
		           	</div>
		   		</div>
				<table width="100%" border="0" cellpadding="5" cellspacing="2">
					<tr>
						<td colspan="4" style="text-align: center; padding-top: 5px;padding-bottom: 5px;">
							<a class="btn btn-mini btn-danger" href='${path }/credit/toEventList.action?where=${where}'">返回</a>
						</td>
					</tr>
				</table>
	</div>
