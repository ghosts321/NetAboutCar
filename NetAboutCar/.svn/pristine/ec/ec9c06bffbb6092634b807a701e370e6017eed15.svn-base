    ...
    oTableInit.Init = function () {
        $('#tradeList').bootstrapTable({
            ...
            columns: [{
                ...
                formatter:function(value, row, index){
                	if(row.is_use == 1){	//启用
	                	var html = "<button type='button' class='btn btn-default' onclick='setState(\""+row.id+"\",\""+row.is_use+"\")'><span class='glyphicon glyphicon-pencil' aria-hidden='true' >停用</button>";
                	}
                	else if(row.is_use == 0){	//停用
...
function toUpdate(id){
	window.parent.baiduMap.toMapUpdate(id);	 //在地图上修改聚集区域
}

// 聚集地点新增
function toAdd(){
	window.parent.baiduMap.toMapAdd();	//在地图上绘制聚集区域
}
            ...
            <div class="panel-body"  style="height:60px;">
                ...
                <form id="formSearch" class="form-horizontal">
                    <div class="form-group" style="margin-top:3px"> 
                        <label class="control-label col-sm-1" style="font-weight: normal;width: 122px;float: left;margin-top: 8px;" for="placeName">聚集地点名称</label>
