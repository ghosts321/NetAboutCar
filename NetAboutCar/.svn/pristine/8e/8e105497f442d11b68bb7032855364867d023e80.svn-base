...
var TableInit = function () {
    ...
    oTableInit.Init = function () {
        $('#tradeList').bootstrapTable({
            url: 'findDrvHandle.action',         //请求后台的URL（*）
            method: 'post',                      //请求方式（*）
            dataType: "json",
            ...
            columns: [{
                ...
                formatter:function(value, row, index){
                  if(row.compn_state==4){
	                	 var html = "<button type='button' disabled='true' class='btn btn-default' onclick='toDrvHandleComit(\""+row.complain_code+"\")'><span class='glyphicon glyphicon-pencil' aria-hidden='true' >处理</button>&nbsp;";
                          }else if(row.compn_state==5){
                        var html = "<button type='button' disabled='true' class='btn btn-default' onclick='toDrvHandleComit(\""+row.complain_code+"\")'><span class='glyphicon glyphicon-pencil' aria-hidden='true' >处理</button>&nbsp;";
                          }else if(row.compn_state==7){
                        var html = "<button type='button' disabled='true' class='btn btn-default' onclick='toDrvHandleComit(\""+row.complain_code+"\")'><span class='glyphicon glyphicon-pencil' aria-hidden='true' >处理</button>&nbsp;";
                          }else{
                       var html = "<button type='button'  class='btn btn-default' onclick='toDrvHandleComit(\""+row.complain_code+"\")'><span class='glyphicon glyphicon-pencil' aria-hidden='true' >处理</button>&nbsp;";
                          }
	                	//html += "<button type='button' class='btn btn-default' onclick='toDrvHandleHistory(\""+row.complain_code+"\")'><span class='glyphicon glyphicon-pencil' aria-hidden='true' >处理历史</button>";
						return html;
                }
            }]
        });
...
function toDrvHandleHistory(complain_code){
	var diag = new top.Dialog();
	 diag.Drag=true;
	 diag.Title ="处理历史";
	 diag.URL = '<%=basePath%>drvcomplaint/showDrvHandleHistory.action?complain_code='+complain_code;
	 diag.Width = 850;
	 diag.Height = 400;
	 diag.CancelEvent = function(){ //关闭事件
		/* if(diag.innerFrame.contentWindow.document.getElementById('zhongxin').style.display == 'none'){
...
function toDrvHandleComit(complain_code){
	var diag = new top.Dialog();
	 diag.Drag=true;
	 diag.Title ="投诉处理";
	 diag.URL = '<%=basePath%>drvcomplaint/showDrvHandle.action?complain_code='+complain_code;
	 diag.Width =800;
	 diag.Height = 600;
	 diag.CancelEvent = function(){ //关闭事件
		/* if(diag.innerFrame.contentWindow.document.getElementById('zhongxin').style.display == 'none'){
...
function todel(){
	...
	if(flag1){
		var url = "<%=basePath%>drvcomplaint/drvFeedBack.action";
