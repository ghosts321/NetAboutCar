...
var TableInit = function () {
    ...
    oTableInit.Init = function () {
        $('#tradeList').bootstrapTable({
            url: 'drvfindAllPunishList.action?id=2&status=0',         //请求后台的URL（*）
            method: 'post',                      //请求方式（*）
            dataType: "json",
            ...
            uniqueId: "punishid",                     //每一行的唯一标识，一般为主键列
            columns: [{
            	checkbox:true,
            },{
                field: 'index',
                ...
                title: '操作',
                formatter:function(value, row, index){
	                	var html = "<button type='button' class='btn btn-default' onclick='toAccept(\""+row.caseid+"\")'><span class='glyphicon glyphicon-pencil' aria-hidden='true' >受理</button>&nbsp;";
						return html;
                }
            }],
...
function toAdd(){
	var diag = new top.Dialog();
	 diag.Drag=true;
	 diag.Title ="新增案件";
	 diag.URL = '<%=basePath%>drvpenalty/generalAddOpen.action';
	 diag.Width = 450;
	 diag.Height = 600;
	...
	<div class="panel-body">
		<div class="page-header" >
			<h1 style="font-size: 15px;">
				...
				<i class="icon-double-angle-right"></i>一般处罚<i class="icon-double-angle-right"></i>受理
				</small>
			</h1>
		</div>
