...
var TableInit = function () {
    ...
    oTableInit.Init = function () {
        $('#tradeList').bootstrapTable({
            url: 'comHandleSelect.action',         //请求后台的URL（*）
            method: 'post',                      //请求方式（*）
            dataType: "json",
            ...
            columns: [{
                ...
                formatter:function(value, row, index){
                	if(row.compn_end != 0){
                	var html = "<a onclick='toEnd(\""+row.allot_depme+"\")'>"+row.compn_end+"</a>";
					return html;
                	}else{
                		return "0";
                	}
...
function toDaichuli(allot_depme){
	...
	var end_time = $("#end_time").val();
	 ...
	 diag.Drag=true;
	 diag.Title ="待处理详情";
	 diag.URL = '<%=basePath%>complaint/comDaichuliList.action?allot_depme='+allot_depme+'&&begin_time='+begin_time+'&&end_time='+end_time;
	 diag.Width = 1200;
	 diag.Height = 700;
...
function toEnd(allot_depme){
	var diag = new top.Dialog();
	var begin_time = $("#begin_time").val();
	var end_time = $("#end_time").val();
	 diag.Drag=true;
	 diag.Title ="已完成详情";
	 diag.URL = '<%=basePath%>complaint/comEndList.action?allot_depme='+allot_depme+'&&begin_time='+begin_time+'&&end_time='+end_time;
	 diag.Width = 1200;
	 diag.Height = 700;
	 diag.CancelEvent = function(){ //关闭事件
		/*  if(diag.innerFrame.contentWindow.document.getElementById('zhongxin').style.display == 'none'){
	...
	<div class="panel-body">
		<div class="page-header" >
			<h1 style="font-size: 15px;">
				...
				<i class="icon-double-angle-right"></i>平台投诉信息处理查询
				</small>	
			</h1>
		</div>
