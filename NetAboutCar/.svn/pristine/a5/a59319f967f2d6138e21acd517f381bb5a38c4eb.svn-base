...
var TableInit = function () {
    ...
    oTableInit.Init = function () {
        $('#tradeList').bootstrapTable({
            url: 'carexamineFindAll.action',         //请求后台的URL（*）
            method: 'post',                      //请求方式（*）
            dataType: "json",
            ...
            uniqueId: "car_illegal_id",                     //每一行的唯一标识，一般为主键列
              columns: [{
                ...
                formatter:function(value, row, index){
                	var html = "";	
                	if(row.status == 2 ){
                		html += "<button type='button' class='btn btn-default' onclick='toAccept(\""+row.car_illegal_id+"\")'><span class='glyphicon glyphicon-pencil' aria-hidden='true' >审核</button>&nbsp;";
                	}else{
                		html += "<button type='button' class='btn btn-default' disabled onclick='toAccept(\""+row.car_illegal_id+"\")'><span class='glyphicon glyphicon-pencil' aria-hidden='true' >审核</button>&nbsp;";
                	}
...
function toAccept(car_illegal_id){
	var diag = new top.Dialog();
	 diag.Drag=true;
	 diag.Title ="违规审核";
	 diag.URL = '<%=basePath%>supervise/carwzwfExamine.action?car_illegal_id=' + car_illegal_id;
	 diag.Width = 800;
	 diag.Height = 700;
	 diag.CancelEvent = function(){ //关闭事件
		/* if(diag.innerFrame.contentWindow.document.getElementById('zhongxin').style.display == 'none'){
