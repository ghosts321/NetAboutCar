...
function deactive(){
	$("#handleBtn").html('吊销'); 
	$("#reason").html('吊销原因：<input type="text" value="" placeholder="吊销原因" />'); 
	$('#poiltImg').hide();
	$('#detailList').show();
	$('#details').modal();
}
function deactive2(){
	$("#handleBtn").html('吊销'); 
	$("#reason").html('吊销原因：<input type="text" value="网约车运输证过期" placeholder="吊销原因" disabled/>'); 
	$('#poiltImg').hide();
	$('#detailList').show();
	$('#details').modal();
}
function cancellation(){
	$("#handleBtn").html('作废'); 
	$("#reason").html('作废原因：<input type="text" value="" placeholder="作废原因" />'); 
	$('#poiltImg').hide();
	$('#detailList').show();
	$('#details').modal();
}
	...
	<div class="page-content">
		...
		<table id="tradeList" class="table table-striped table-bordered table-hover">
			...
			<tr>
				...
				<td class="center">
					<button type="button" class='btn btn-mini btn-warning' onclick='print()'>打印</button>&nbsp;
					<button type="button" class='btn btn-mini btn-danger' onclick='deactive()'>吊销</button>&nbsp;
					<button type="button" class='btn btn-mini btn-purple' onclick='cancellation()'>作废</button>
				</td>
			</tr>
			<tr>
				...
				<td class="center">
					<button type="button" class='btn btn-mini btn-warning' onclick='print()'>打印</button>&nbsp;
					<button type="button" class='btn btn-mini btn-danger' onclick='deactive2()'>吊销</button>&nbsp;
					<button type="button" class='btn btn-mini btn-purple' onclick='cancellation()'>作废</button>
				</td>
			</tr>
   		</table>
		...
		<div class="modal-dialog">
			<div class="modal-content">
				...
				<div class="modal-body" style="height:300px">
					<div style="text-align:center" id="poiltImg"><img src="<%=basePath %>static/images/poilt.png" width="500" alt="驾驶员营运执照"></div>
					<div id="detailList">
				...
				<div class="modal-footer">
					<div id="reason" style="float: left;"></div>
					<button id="handleBtn" type="button" class="btn btn-primary">打印</button>
