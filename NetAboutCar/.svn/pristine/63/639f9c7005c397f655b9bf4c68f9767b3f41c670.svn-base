...
<form action="<%=basePath %>pilotApply/authentication.action" name="form1" id="form1" method="post" >
		...
		<table border="1" class="table">
			...
			<tr>
				<th style="text-align: right;width: 100px;">案件承办人：</th>
				<td  style="width:200px;">${drvPlantCaseInfo.caseuserid }</td>
			...
			<tr>
				<th style="text-align: right;width: 100px;">处罚类型：</th>
				<td colspan="3">
				<c:choose>
				     <c:when test="${drvPlantCaseInfo.punishtype=='0001'}">口头警告</c:when>
				      <c:when test="${drvPlantCaseInfo.punishtype=='0002'}">两百元以下罚款</c:when>
				      <c:when test="${drvPlantCaseInfo.punishtype=='0003'}">暂扣或吊销驾驶证</c:when>
				</c:choose></td>			
			</tr>
			<tr height="30px">
