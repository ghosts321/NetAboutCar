...
$(function() {
	...
	if(msg=="success"){
		...
		window.parent.baiduMap.toMapSet("add");	//关闭绘图工具条，弹出列表窗口
	}else if(msg=="failed"){
		alert("保存失败！");
		
		window.parent.baiduMap.toMapSet("add");	//关闭绘图工具条，弹出列表窗口
	}else if(msg=="保存失败：该区域已存在！"){
		alert(msg);
		
		window.parent.baiduMap.toMapSet("add");	//关闭绘图工具条，弹出列表窗口
	}else if(msg=="保存失败：系统未获取到区域经纬度，请重新增加区域！"){
		alert(msg);
...
<body>
	<div class="container">
		<form action="addAssemblePlaceSetTotal.action" method="post" name="form" id="form">
