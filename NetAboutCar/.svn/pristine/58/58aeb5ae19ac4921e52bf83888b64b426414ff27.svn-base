...
<body>
<div class="container">
<form action="<%=basePath %>credit/comEventReview.action" name="form1" id="form1" method="post" enctype ="multipart/form-data">
	<div id="zhongxin">
        ...
        <c:choose>
			<c:when test="${comEventReview.review_status == '已审核'}">
				<div style="padding-top:30px;"></div>
		        <div class="panel panel-default">
		            <div class="panel-body">
		            ...
		            <div class="panel-body">
		                ...
		                <table width="100%" border="1" cellpadding="5" cellspacing="2" class="table">
							<tr>
								...
								<td colspan="3">
									<c:choose>
										<c:when test="${null == comEventReview.result_file || comEventReview.result_file == ''}">无</c:when>
										<c:otherwise><a href="${comEventReview.result_file}">点击下载</a></c:otherwise>
									</c:choose>
								</td>
							</tr>
							<tr>
							    <th style="text-align: right;" >处理结论：</th>
							    <td colspan="3"><textarea style="width: 400px;height:50px" cols="40" rows="5">${comEventReview.review_result}</textarea></td>
							</tr>
		                </table>
		           	</div>
		   		</div>
				...
				<table width="100%" border="0" cellpadding="5" cellspacing="2">
					<tr>
						<td colspan="4" style="text-align: center; padding-top: 5px;padding-bottom: 5px;">
							...
							<input type="hidden" id="creditcode" name="creditcode" value="${platform.creditCode}" />
