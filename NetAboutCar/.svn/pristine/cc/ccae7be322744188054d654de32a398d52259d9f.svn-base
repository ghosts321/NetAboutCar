    ...
    oTableInit.Init = function () {
        $('#tradeList').bootstrapTable({
            ...
            columns: [{
                ...
                title: '操作',
                formatter:function(value, row, index){
	                	var html = "<button type='button' class='btn btn-default' onclick='toUpdate(\""+row.abnormity_id+"\")'><span class='glyphicon glyphicon-pencil' aria-hidden='true' >修改</button>";
						return html;
                }
            }]
        });
...
function toAdd(){
	var diag = new top.Dialog();
	 diag.Drag=true;
	 diag.Title ="增加运价";
	 diag.URL = '<%=basePath%>monitor/abnormityPriceAdd.action';
	 diag.Width = 400;
	 diag.Height = 300;
...
function toUpdate(abnormity_id){
	var diag = new top.Dialog();
	 diag.Drag=true;
	 diag.Title ="修改运价";
	 diag.URL = '<%=basePath%>monitor/abnormityPriceUpdate.action?abnormity_id='+abnormity_id;
	 diag.Width = 400;
	 diag.Height = 300;
	 diag.CancelEvent = function(){ //关闭事件
		if(diag.innerFrame.contentWindow.document.getElementById('zhongxin').style.display == 'none'){
	...
	<div class="panel-body">
		<div class="page-header" >
			<h1 style="font-size: 15px;">
				...
				<i class="icon-double-angle-right"></i>异常运价设定
				</small>
			</h1>
		</div>
            ...
            <div class="panel-body">
                ...
                <form id="formSearch" class="form-horizontal">
                    <div class="form-group" style="margin-top:15px">
                        <label class="control-label col-sm-1" style="font-weight: normal;" for="region_code">行政区划代码</label>
                        <div class="col-sm-3" style="padding-right: 5px;padding-left: 0px">
							<input type="text" class="form-control" id="region_code" placeholder="请输入行政区划代码"/>
                        </div>
                        <label class="control-label col-sm-1" style="font-weight: normal;" for="fare_type">运价类型编号</label>
                        <div class="col-sm-3" style="padding-right: 5px;padding-left: 0px">
							<input type="text" class="form-control" id="fare_type" placeholder="请输入运价类型编号" />
                        </div>
