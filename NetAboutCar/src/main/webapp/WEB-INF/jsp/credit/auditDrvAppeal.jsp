...
function submitForm(r,obj){
	...
	if(dealResult == ""){
		...
		if(confirm($(obj).text()+"，确定要提交此处理结论？")){
			$('#form1').attr('action',$('#form1').attr('action')+'?appealResult='+r);
			$("#form1").ajaxSubmit({
...
<div class="container">
<form action="<%=basePath %>credit/driver/appeal/audited.action" name="form1" id="form1" method="post" enctype ="multipart/form-data">
	<div id="zhongxin">
		            ...
		            <div class="panel-body">
		                ...
		                <table width="100%" border="1" cellpadding="5" cellspacing="2" class="table">
							<tr>
							    ...
							    <td colspan="3">${ca.appealText }</td>
							</tr>
							<tr>
								<th style="text-align: right;" >申诉材料：</th>
								<td colspan="3">
									<c:choose>
										<c:when test="${null == ca.appealFile || ca.appealFile== ''}">无</c:when>
										<c:otherwise><a href="${ca.appealFile}">点击下载</a></c:otherwise>
									</c:choose>
								</td>
							</tr>
							<tr>
							    <th style="text-align: right;" >申诉时间：</th>
							    <td colspan="3"><fmt:formatDate  value="${ca.appealDate }" type="both" pattern="yyyy-MM-dd hh:mm:ss" /></td>
							</tr>
							<tr>
							    <th style="text-align: right;" >处理结论：</th>
							    <td colspan="3"><textarea style="width: 400px;height:100px" cols="40" rows="5"
							    id="dealResult" name="dealResult" 
							    placeholder="请输入申诉审核意见和结论,800字以内"></textarea></td>
							</tr>
		                </table>
		           	</div>
		   		</div>
				...
				<table width="100%" border="0" cellpadding="5" cellspacing="2">
					<tr>
						<td colspan="4" style="text-align: center; padding-top: 5px;padding-bottom: 5px;">
							<input type="hidden" id="rec_no" name="rec_no" value="${cad.recNo}" />
							<input type="hidden" id="id" name="id" value="${cad.id}" />
							<button type="button" class="btn btn-mini btn-success" onclick="submitForm(1,this);">同意申诉，撤销记分</button>&nbsp;&nbsp;&nbsp;
							<button type="button" class="btn btn-mini btn-danger" onclick="submitForm(0,this);">申诉无效，维持记分</button>&nbsp;&nbsp;&nbsp;
