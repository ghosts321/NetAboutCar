...
<form  action="bcwddy.action" name="contentForm" id="contentForm" method="post" enctype="multipart/form-data">
	...
	<input type=hidden name="dimType" id="dimType" value="{{jdd.dimType}}"/>
		...
		<table >
		    <tr class='title-tr'>
			    <td colspan="2" align="left"><div>添加维度定义:{{sjzl.dataTitle}}</div>
			    <div class="title-comment">维度定义功能主要完成对数据字段的解译，包括了维度字段和值字段。</div></td>
			    <td>&nbsp;</td>
			</tr>
			<tr>
				<td style="text-align: right;">字段类型：</td>
				<td align="left"><select ng-model="jdd.fieldType" ng-options='p.value as p.text for p in fieldTypes'></select>
				</td>
			</tr>
			<tr>
				<td style="text-align: right;">维度/值代码：</td>
				<td align="left"><div><input type="text" ng-change='doChangeDimCode()' ng-model='jdd.dimCode' name="dimCode" id="dimCode" value="" placeholder="维度代码"/></div>
			...
			<tr>
				...
				<td align="left"><div>
				<input type="text" ng-model='jdd.dimFieldname' name="dimFieldname" id="dimFieldname" value="" placeholder="jcfx_data表字段，如dim2或value3"/></div>
			...
			<tr>			    
				<td colspan="2" style="text-align: center; padding-top: 30px;">
					...
					<a class="btn btn-mini btn-danger" href='${path }/jcfx/wddy.action?dataCode=${jdc.dataCode}'>取消</a>
				</td>
			</tr>
		</table>
		</div>
