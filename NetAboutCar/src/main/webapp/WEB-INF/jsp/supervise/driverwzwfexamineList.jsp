...
var TableInit = function () {
    ...
    oTableInit.Init = function () {
        $('#tradeList').bootstrapTable({
            url: 'driverexamineFindAll.action',         //请求后台的URL（*）
            method: 'post',                      //请求方式（*）
            dataType: "json",
            ...
            uniqueId: "drv_illegal_id",                     //每一行的唯一标识，一般为主键列
              columns: [{
                ...
                formatter:function(value, row, index){
                	var html = "";	
                	if(row.status == 2 ){
                		html += "<button type='button' class='btn btn-default' onclick='toAccept(\""+row.drv_illegal_id+"\")'><span class='glyphicon glyphicon-pencil' aria-hidden='true' >审核</button>&nbsp;";
                	}else{
                		html += "<button type='button' class='btn btn-default' disabled onclick='toAccept(\""+row.drv_illegal_id+"\")'><span class='glyphicon glyphicon-pencil' aria-hidden='true' >审核</button>&nbsp;";
                	}
