...
var TableInit = function () {
    ...
    oTableInit.Init = function () {
        $('#tradeList').bootstrapTable({
            url: 'carillegalSearch.action',         //请求后台的URL（*）
            method: 'post',                      //请求方式（*）
            dataType: "json",
            ...
            uniqueId: "car_illegal_id",                     //每一行的唯一标识，一般为主键列
              columns: [{
                ...
                formatter:function(value, row, index){
                	var html = "";	            	
                		html += "<button type='button' class='btn btn-default' onclick='toAccept(\""+row.car_illegal_id+"\")'><span class='glyphicon glyphicon-pencil' aria-hidden='true' >详细</button>&nbsp;";
						return html;
                }
            }]
        });
...
function toAccept(car_illegal_id){
	var diag = new top.Dialog();
	 diag.Drag=true;
	 diag.Title ="违规详情";
	 diag.URL = '<%=basePath%>supervise/carillegalSearchDetails.action?car_illegal_id=' + car_illegal_id;
	 diag.Width = 700;
	 diag.Height = 400;
	 diag.CancelEvent = function(){ //关闭事件
		/* if(diag.innerFrame.contentWindow.document.getElementById('zhongxin').style.display == 'none'){
