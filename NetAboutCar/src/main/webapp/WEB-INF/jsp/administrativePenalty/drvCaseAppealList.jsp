...
var TableInit = function () {
    ...
    oTableInit.Init = function () {
        $('#tradeList').bootstrapTable({
            url: 'drvappealFindAll.action',         //请求后台的URL（*）
            method: 'post',                      //请求方式（*）
            dataType: "json",
...
function toAccept(caseid){
	var diag = new top.Dialog();
	 diag.Drag=true;
	 diag.Title ="申诉处理";
	 diag.URL = '<%=basePath%>plantcaseappeal/drvcaseAppeal.action?caseid=' + caseid;
	 diag.Width = 1000;
	 diag.Height = 600;
	 diag.CancelEvent = function(){ //关闭事件
		if(diag.innerFrame.contentWindow.document.getElementById('zhongxin').style.display == 'none'){
...
function toDetail(caseid){
	var diag = new top.Dialog();
	 diag.Drag=true;
	 diag.Title ="案件详情";
	 diag.URL = '<%=basePath%>plantcaseappeal/drvappealDetail.action?caseid=' + caseid;
	 diag.Width =1000;
	 diag.Height = 600;
	 diag.CancelEvent = function(){ //关闭事件
		if(diag.innerFrame.contentWindow.document.getElementById('zhongxin').style.display == 'none'){
