...
var TableInit = function () {
    ...
    oTableInit.Init = function () {
        $('#tradeList').bootstrapTable({
            url: 'findaccept.action',         //请求后台的URL（*）
            method: 'post',                      //请求方式（*）
            dataType: "json",
            ...
            columns: [{
                ...
                title: '操作',
                formatter:function(value, row, index){
	                	var html = "<button type='button' class='btn btn-default' onclick='toaccept(\""+row.complain_code+"\")'><span class='glyphicon glyphicon-pencil' aria-hidden='true' >受理</button>&nbsp;";
 	                	//html += "<button type='button' class='btn btn-default' onclick='toaccepthistory(\""+row.complain_code+"\")'><span class='glyphicon glyphicon-pencil' aria-hidden='true' >处理历史</button>";
						return html;
                }
            }]
        });
...
function toaccepthistory(complain_code){
	var diag = new top.Dialog();
	 diag.Drag=true;
	 diag.Title ="处理历史";
	 diag.URL = '<%=basePath%>complaint/showaccepthistory.action?complain_code='+complain_code;
	 diag.Width = 800;
	 diag.Height = 400;
	 diag.CancelEvent = function(){ //关闭事件
		/* if(diag.innerFrame.contentWindow.document.getElementById('zhongxin').style.display == 'none'){
...
function toaccept(complain_code){
	var diag = new top.Dialog();
	 diag.Drag=true;
	 diag.Title ="投诉受理";
	 diag.URL = '<%=basePath%>complaint/showaccept.action?complain_code='+complain_code;
	 diag.Width =750;
	 diag.Height = 600;
	 diag.CancelEvent = function(){ //关闭事件
		/* if(diag.innerFrame.contentWindow.document.getElementById('zhongxin').style.display == 'none'){
