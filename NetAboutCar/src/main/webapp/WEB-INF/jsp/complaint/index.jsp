...
var TableInit = function () {
    ...
    oTableInit.Init = function () {
        $('#tradeList').bootstrapTable({
            url: 'findAllComplaint.action',         //请求后台的URL（*）
            method: 'post',                      //请求方式（*）
            dataType: "json",
...
function tocomit(complain_code){
	var diag = new top.Dialog();
	 diag.Drag=true;
	 diag.Title ="投诉提交";
	 diag.URL = '<%=basePath%>complaint/tocomit.action?complain_code='+complain_code;
	 diag.Width = 750;
	 diag.Height = 500;
	 diag.CancelEvent = function(){ //关闭事件
		/* if(diag.innerFrame.contentWindow.document.getElementById('zhongxin').style.display == 'none'){
...
function toUpdate(complain_code){
	var diag = new top.Dialog();
	 diag.Drag=true;
	 diag.Title ="投诉修改";
	 diag.URL = '<%=basePath%>complaint/showUpdate.action?complain_code='+complain_code;
	 diag.Width =1000;
	 diag.Height = 700;
	 diag.CancelEvent = function(){ //关闭事件
		/* if(diag.innerFrame.contentWindow.document.getElementById('zhongxin').style.display == 'none'){
