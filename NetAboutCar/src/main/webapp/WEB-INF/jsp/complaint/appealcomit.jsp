...
//保存校验
function save(id){
    document.addForm.action='<%=basePath%>complaint/appealsuccess.action?id='+id;
	$("#addForm").submit();
	$("#zhongxin").hide();
	$("#zhongxin2").show();
}
...
<form  name="addForm" id="addForm" method="post" enctype="multipart/form-data" >
		...
		<table  class="table">	      
			     ...
			     <c:if test="${complainappeal.appeal_file!=null && complainappeal.appeal_file!=''}">
				 <tr height="40">
								<td align="right">申诉文件：</td>
								<td  align="left">
							         <div class="box">
										<a href="${complainappeal.appeal_file}">点击下载</a>
			    ...
			    <c:if test="${complainappeal.appl_content!=null && complainappeal.appl_content!=''}">
			    <td>${complainappeal.appl_content} 
			    </td>
			    </c:if>
			    <c:if test="${complainappeal.appl_content==null && complainappeal.appl_content==''}">
			     <td>无</td>
			...
			<tr>
			    ...
			    <td><textarea name="appl_dis_idea" id="appl_dis_idea" style="width:200px;height:80px;"></textarea>
			 ...
			 <tr>
				<td colspan="2" style="text-align: center; padding-top: 20px;" colspan=4>
					<button type="button" value="1" class="btn btn-mini btn-primary"  onclick="save('1')">申诉通过</button>&nbsp;&nbsp;&nbsp;
					<button type="button" value="2" class="btn btn-mini btn-primary"  onclick="save('2')">申诉不通过</button>&nbsp;&nbsp;&nbsp;
