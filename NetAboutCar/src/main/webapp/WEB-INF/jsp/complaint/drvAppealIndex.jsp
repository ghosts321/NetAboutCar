...
var TableInit = function () {
    ...
    oTableInit.Init = function () {
        $('#tradeList').bootstrapTable({
            url: 'findDrvAppeal.action',         //请求后台的URL（*）
            method: 'post',                      //请求方式（*）
            dataType: "json",
            ...
            columns: [{
                ...
                title: '操作',
                formatter:function(value, row, index){
	                	var html = "<button type='button' class='btn btn-default' onclick='toDrvAppeal(\""+row.complain_code+"\")'><span class='glyphicon glyphicon-pencil' aria-hidden='true' >受理</button>&nbsp;";
	                	html += "<button type='button' class='btn btn-default' onclick='drvAppealDetail(\""+row.complain_code+"\")'><span class='glyphicon glyphicon-pencil' aria-hidden='true' >处理详情</button>";
						return html;
                }
            }]
        });
...
function drvAppealDetail(complain_code){
	var diag = new top.Dialog();
	 diag.Drag=true;
	 diag.Title ="处理详情";
	 diag.URL = '<%=basePath%>drvcomplaint/drvAppealDetail.action?complain_code='+complain_code;
	 diag.Width = 750;
	 diag.Height = 500;
	 diag.CancelEvent = function(){ //关闭事件
		/* if(diag.innerFrame.contentWindow.document.getElementById('zhongxin').style.display == 'none'){
...
function toDrvAppeal(complain_code){
	var diag = new top.Dialog();
	 diag.Drag=true;
	 diag.Title ="申诉处理";
	 diag.URL = '<%=basePath%>drvcomplaint/showDrvAppeal.action?complain_code='+complain_code;
	 diag.Width =650;
	 diag.Height =500;
	 diag.CancelEvent = function(){ //关闭事件
		/* if(diag.innerFrame.contentWindow.document.getElementById('zhongxin').style.display == 'none'){
	...
	<div class="panel-body">
		<div class="page-header" >
			<h1 style="font-size: 15px;">
				申诉管理<small style="font-size: 15px">
				<i class="icon-double-angle-right"></i>驾驶员申诉处理
				</small>
